...
class BusService {
    ...
    /**
     * Asynchonously gets the current list of bus stops as a set. 
     * Query the web server if bus stops are not retrieved before.
     * @return A promised set of bus stops that this bus services serves.
     */
    public CompletableFuture<Set<BusStop>> getBusStops() {
        // A seperate CompletableFuture is required so that the Scanner can be
        // closed after use
        // Queries the API for the bus stops the bus service goes to
        CompletableFuture<Scanner> sc = BusAPI.getBusStopsServedBy(serviceId)
            .thenApply(x -> new Scanner(x));

        CompletableFuture<Set<BusStop>> stops = sc.thenApply(scan -> 
                scan.useDelimiter("\n")
                .tokens()
                .map(line -> line.split(","))
    ...
    /**
     * Asynchronously returns a list of bus stops matching a given name.
     * @param  name Name (possibly partial) of a bus stop.
     * @return A promised list of bus stops matching the given name.
     */
    public CompletableFuture<Set<BusStop>> findStopsWith(String name) {
        return getBusStops().thenApply(stops -> 
                stops.stream()
